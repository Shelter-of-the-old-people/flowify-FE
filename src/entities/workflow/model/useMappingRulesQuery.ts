...
export const useMappingRulesQuery = (
  enabledOrOptions?:
    | boolean
    | QueryPolicyOptions<
        Awaited<ReturnType<typeof workflowApi.getMappingRules>>
      >,
) => {
  const options = resolveQueryPolicyOptions(enabledOrOptions);

  return useQuery({
    queryKey: workflowKeys.mappingRules(),
    queryFn: workflowApi.getMappingRules,
