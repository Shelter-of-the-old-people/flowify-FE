...
현재 FE에서는 아래 범위를 반영했다.

- `file_upload_auto_share` 카테고리 라벨 추가
- 대표 템플릿 3종 description override 추가
- 템플릿 상세의 runtime note 추가
- Slack `channel_picker`를 sink target option 목록 선택 UX로 연결
- Notion `page_picker`를 sink target option 목록 선택 UX로 연결
- Google Drive `folder_picker`의 새 폴더 만들기 UX를 그대로 유지

현재 1차 범위에서 지원하는 선택 UX는 다음과 같다.

- Google Drive: 폴더 목록 선택 + 새 폴더 만들기
- Slack: 기존 공개 채널 목록 선택
- Notion: 기존 공유 페이지 / 접근 가능한 페이지 목록 선택

현재 1차 범위에서 지원하지 않는 것은 다음과 같다.

- Slack 설정 패널 내부에서 새 채널 생성
- Notion 설정 패널 내부에서 새 부모 페이지 생성
...
4. Slack / Notion picker 입력 안내 문구와 metadata 표시를 실제 목록 응답 기준으로 최종 정리
